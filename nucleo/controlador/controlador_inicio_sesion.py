    ...
    def inner_decorator(f):
        ...
        def wrapped(*args, **kwargs):
            ...
            try:
                ...
                expiracion = datetime.datetime.strptime(
                    data["expiracion"], '%Y-%m-%d %H:%M:%S.%f')
                ...
                usuario_actual = db.session.query(Usuario).filter(
                    Usuario._id == data['public_id']).first()
                if not usuario_actual:
                    raise Exception('Token inexistente.')

                rol = db.session.query(Rol_usuario).filter(
                    Rol_usuario._id == usuario_actual.rol_usuario).first()

                sesion_iniciada = db.session.query(Inicio_sesion).filter(
                    Inicio_sesion.usuario == data['public_id'], Inicio_sesion.estatus == 'Activo').first()

                if (rol.nombre not in rol_admitido) and rol_admitido != 'Any':
